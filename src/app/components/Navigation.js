...
export default function Navigation() {
	...
	const createMatrixRain = () => {
		...
		matrixContainer.className = "fixed inset-0 pointer-events-none z-[60] overflow-hidden";
	...
	const createGlitchEffect = () => {
		...
		glitchText.className = "fixed inset-0 flex items-center justify-center pointer-events-none z-[60]";
	...
	const createCelebrationEffect = () => {
		...
		celebration.className = "fixed inset-0 pointer-events-none z-[60] flex items-center justify-center";
    ...
    `;
        ...
        width: 10px;
        height: 10px;
        background: ${["#ff1493", "#00ffff", "#50fa7b"][Math.floor(Math.random() * 3)]};
        border-radius: 50%;
        pointer-events: none;
        z-index: 60;
	...
	const createHackerMode = () => {
		...
		hackerOverlay.className = "fixed inset-0 bg-black/90 pointer-events-none z-[60] flex items-center justify-center";
		...
		<nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-foreground/10">
					...
					<div className="hidden md:block">
						<div className="ml-10 flex items-baseline space-x-8">
							<button
								...
								className="hover:text-foreground transition-colors text-sm font-medium interactive-element relative group"
							>
			...
			<div className={`md:hidden ${isMenuOpen ? "block" : "hidden"}`}>
				<div className="px-2 pt-2 pb-3 space-y-1 bg-background border-t border-foreground/10">
					<button
						onClick={(e) => handleNavClick(e, "about")}
						className="block px-3 py-2 text-base font-medium hover:bg-foreground/10 hover:text-foreground w-full text-left transition-colors interactive-element relative group"
					>
						<span className="relative">
							About
